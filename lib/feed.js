...
function parseString(xml, options, callback) {
    ...
    if (xml.split('<').length >= 3) {
        var parser = new xml2js.Parser({trim: false, normalize: true, mergeAttrs: true, explicitCharkey: true});
        parser.addListener('end', function (jsonDOM) {
            if (jsonDOM) {
                //console.log(jsonDOM.rss.channel[0]._);
...
function formatRSS(json) {
    ...
    if (channel.description) {
        output.description = channel.description[0]._;
    }
    if (channel.link) {
        output.url = channel.link[0]._;
    }
    if (channel.lastBuildDate) {
        output.last_modified = channel.lastBuildDate[0]._;
    }
    if (channel.pubDate) {
        output.update = channel.pubDate[0]._;
    }
    if (channel.ttl) {
...
function formatATOM(json) {
    ...
    if (channel.id) {
        output.id = channel.id[0]._;
    }

    if (channel.updated) {
        output.last_modified = new Date(channel.updated[0]._).toString();
    }

    if (channel.author) {
        output.author = channel.author[0].name[0]._;
    }
    ...
    if (channel.entry) {
        ...
        _.each(channel.entry, function (val, index) {
            ...
            if (val.link) {
                if (_.isArray(val.link)) {
                    _.each(val.link, function (val, i) {
                        if (val.rel === 'self' || val.rel === 'alternate') {
                            link = val.href;
                        }
                    });
                } else {
            ...
            if (val.published) {
                //lets try basis js date parsing for now
                obj.published_at = Date.parse(val.published[0]._);
                obj.time_ago = DateHelper.time_ago_in_words(obj.published_at);
            }

            if (val.updated) {
                obj.last_modified = new Date(val.updated[0]._).toString();
            }

            if (val.author) {
                obj.author = val.author[0].name[0]._;
            }
